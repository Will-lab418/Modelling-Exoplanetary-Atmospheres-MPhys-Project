...
# This script reads outputs for a P-T profile and chemical abundances from ATMO, VULCAN, PACT, and FRECKLL
# and plots them together for easier comparison.

# Plots P-T profiles for ATMO, VULCAN, PACT, and FRECKLL

def plot_pt(atmo_file, vulcan_file, pact_file, freckll_file, tit = 'HD209 P-T'):

    linewidth = 1.2

    linestyles = ['-', '--', '-.', ':']
    files = [atmo_file, vulcan_file, pact_file, freckll_file]
    models = ['ATMO', 'VULCAN', 'PACT', 'FRECKLL']
    ...
    def read_data(model, fpath):
        ...
        if model == 'ATMO':
            ...
            nc = Dataset(fpath, 'r')
            tt = np.array(nc.variables['temperature'][:], dtype = float)
            pp = np.array(nc.variables['pressure'][:], dtype = float) / 1000 # converts milibar to bar
            nc.close()
            return tt, pp
...
def plot_abundances(atmo_file, vulcan_file, pact_file, freckll_file, tit = 'HD 209 Chemical Kinetics', species = ['H2O', 'H', 'CO', 'CH4', 'NH3', 'H2', 'He']):

    linewidth = 1.2
    freckll_time_index = -1
    pact_log10 = True
    ...
    # read FRECKLL
    nc_freckll = Dataset(freckll_file, 'r')
    p_freckll = np.array(nc_freckll.variables['pressure'][:], dtype=float) 
    freckll_vmrs = np.array(nc_freckll.variables['vmrs'][freckll_time_index, :, :], dtype=float)
    freckll_species = decode_species(nc_freckll.variables['species_formula'][:])
    nc_freckll.close()

    freckll_idx = {}
    for i, m in enumerate(freckll_species):
        if m not in freckll_idx:
            freckll_idx[m] = i
    ...
    for i, mol in enumerate(species):
        ...
        if mol in freckll_idx:
            ax.plot(freckll_vmrs[freckll_idx[mol], :], p_freckll, linewidth = linewidth, linestyle = ':', color = c)
        # FRECKLL USES A DIFFERENT NAME FOR NH3 (H3N)
        elif mol == 'NH3' and 'H3N' in freckll_idx:
            ax.plot(freckll_vmrs[freckll_idx['H3N'], :], p_freckll, linewidth=linewidth, linestyle=':', color=c)
    ...
    ax.legend(model_handles, ['ATMO', 'VULCAN', 'PACT', 'FRECKLL'], loc = 'lower left', bbox_to_anchor = (0, 0.30), fontsize = 14, frameon = False)
